...
# Local build, not `docker compose pull`: a pulled GHCR image only exists
# for tagged releases and lagged behind plain `main` commits, which is what
# made this step's own freshness check below fail and got the whole script
# deleted for a while (see commit 87ff3bc) rather than fixed. Building from
# the code that's actually on disk here is what step 5 assumes.
