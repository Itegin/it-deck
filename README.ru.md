# IT-Deck

![Логотип IT-Deck](frontend/icons/icon-180.png)

Твой телефон становится пультом для Windows-ПК — тот, что всегда с тобой,
пока ты сидишь за компьютером, или запасной, который стоит на подставке. Один
`.exe` на компьютере, QR-код — и телефон работает как Stream Deck, который ещё
и показывает, что на ПК происходит на самом деле. На телефон ничего ставить не
нужно: всё работает в браузере.

**Для кого:** геймеры и стримеры, удалёнщики, студенты и школьники — и все,
кому одно касание удобнее трёх кликов.

**[English version](README.md)**

| Дека на телефоне | Studio на ПК |
| --- | --- |
| <img src="docs/screenshots/dashboard.png" alt="Дека IT-Deck на телефоне: две точки — две деки, виджет часов и погоды, красная плитка микрофона во время мьюта, громкость 62 %, Audio Switch с текущим устройством, подсвеченный VPN, виджет нагрузки ПК (процессор, память, скорость сети), плитка VALORANT и панель быстрого запуска внизу: Telegram, Discord, YouTube, Steam, Claude" width="300"> | ![Studio: живой предпросмотр деки рядом с редактором плитки-программы VALORANT и логотипами популярных приложений, игр и лаунчеров; в верхней панели кнопки Decks, Access и What's new](docs/screenshots/studio.png) |

Плитки показывают реальное состояние, а не просто нажимаются: на скриншоте
микрофон **в мьюте** (красный), громкость — **62 %**, Audio Switch называет
**текущее устройство вывода**, **VPN подсвечен**, потому что клиент
запущен, а **виджет нагрузки ПК** показывает процессор, память и скорость
сети. Точки сверху — это **деки**: следующая открывается свайпом. Внизу — **панель быстрого запуска**: сайты и программы в одно
касание (когда телефон лежит горизонтально, она уезжает влево). **Studio**
(справа) — место, где дека собирается: клик по плитке в
живом предпросмотре открывает её настройки, `+` добавляет новую,
перетаскивание двигает.

Четыре темы, каждая на тёмном и светлом фоне:

![Одна и та же дека в темах Flat, Pastel, Glossy и Liquid Glass](docs/screenshots/themes.png)

## Быстрый старт

Всё работает на том же ПК, которым управляешь. Ни сервера, ни Docker, ни
Python на этой машине.

1. **Скачай `ITDeck.exe`** из
   [последнего релиза](https://github.com/Itegin/it-deck/releases/latest) и
   запусти. В первый раз короткое обучение проведёт по настройке, а дальше
   остаётся это окно:

   <img src="docs/screenshots/window.png" alt="Окно IT-Deck в тёмном стеклянном стиле Studio (на скриншоте — английский интерфейс; на русской Windows окно по-русски): шапка «IT-Deck работает», шаг 1 — QR-код, ссылка, «Скопировать ссылку» и «Новый PIN для телефона», шаг 2 открывает Studio, шаг 3 объясняет «Свернуть» и «Выйти» и предлагает запускать IT-Deck вместе с Windows; «Обучение» повторяет первый тур" width="535">

2. **Наведи камеру телефона на QR-код** — телефон должен быть в той же сети
   Wi-Fi. Это единственный обязательный шаг: токен лежит в ссылке, телефон
   его запоминает. Если адрес не открывается, под ссылкой окно перечисляет
   остальные адреса этого ПК.

3. **По желанию:** все плитки работают сразу, кроме **VPN** — ей нужно
   указать, какую программу запускать. См.
   [Настройка плитки VPN](#настройка-плитки-vpn).

Кнопка **«Выйти»** в этом окне останавливает IT-Deck, **«Свернуть»** —
оставляет работать с подключённым телефоном. Всё, что IT-Deck запустил по
твоей команде (включая VPN-клиент), продолжает работать в обоих случаях.

При первом запуске он ещё кладёт ярлык **IT-Deck** на рабочий стол (дальше
запускай с него), создаёт `%LOCALAPPDATA%\IT-Deck\config.env` с двумя токенами
(по умолчанию оба `admin`) и портом (`49732` или следующий свободный) и
поднимает бэкенд и Windows-агент отдельными процессами. Консоли нет — логи
лежат в `%LOCALAPPDATA%\IT-Deck\logs\`. Конфиг и токены переживают
перезапуски: если удалить `config.env` или сменить порт, ссылка, сохранённая
на телефоне, перестанет работать — тогда открой на нём новую один раз.

**Что нужно:** Windows-ПК для управления (агент использует COM-API звука
Windows и работает в интерактивном сеансе пользователя) и телефон с браузером
в той же локальной сети.

<details>
<summary>Или собери exe сам</summary>

```
git clone https://github.com/Itegin/it-deck.git
cd it-deck
standalone\build.ps1
```

На машине, которая **собирает**, нужен Python 3.7–3.12 (`comtypes` для
управления звуком не поддерживает 3.13/3.14). Результат —
`standalone\dist\ITDeck.exe`, ~29 МБ, которому для запуска не нужно уже
ничего.

</details>

## Плитки

Нажатие отправляет команду по WebSocket Windows-агенту, тот её выполняет и
отвечает. Любая команда завершается за 5 секунд — успех, ошибка или таймаут, —
и плитка показывает, чем именно. Долгое нажатие открывает меню с **Force
Stop** для зависшего процесса.

Свежая установка создаёт восемь плиток, и за каждой стоит реальная команда:

| Плитка | Что делает |
| --- | --- |
| **Terminal** | Запускает приложение — по умолчанию Windows Terminal |
| **Mic** | Включает/выключает микрофон; красная во время мьюта |
| **Volume** | Перетаскиванием задаёт громкость вывода |
| **Headphones** | Включает/выключает звук динамиков |
| **Audio Switch** | Переключает два устройства вывода и называет текущее |
| **Screenshot** | Снимает главный монитор в буфер обмена **ПК** |
| **VPN** | Запускает VPN-клиент, подсвечена, пока он работает — **сначала нужен путь** |
| **Close Agent** | Останавливает агент; вернуть — кнопкой **«Запустить агент»** в окне IT-Deck |

В Studio есть и другие:

| Плитка | Что делает |
| --- | --- |
| **Время и погода** | Собственные часы и дата телефона плюс погода для выбранного города (Open-Meteo, без API-ключа) |
| **Нагрузка ПК** | Живые полоски загрузки процессора и памяти, скорость сети |
| **Горячие клавиши** | Нажимает сочетание клавиш на ПК, например `ctrl+shift+m` — выключить микрофон в Discord |
| **Медиаклавиша** | Пауза/воспроизведение, следующий и предыдущий трек, стоп, громкость |
| **Питание** | Блокировка, сон, выключение или перезагрузка; сначала спрашивает |
| **Отправить текст** | Набираешь на телефоне, вставляешь на ПК: текст попадает в его буфер обмена |
| **Вкл/выкл программу** | Запускает программу или закрывает её, если та уже работает |

Любой плитке можно включить **«Спрашивать перед запуском»** — тогда телефон
сначала покажет «Запустить / Отмена». Некоторые игры и античиты игнорируют
программные нажатия клавиш, поэтому внутри них плитка горячих клавиш может
не сработать.

**Быстрый запуск.** **«Открыть сайт»** открывает любой адрес в браузере ПК
(готовые пресеты: Telegram Web, Discord, YouTube, ChatGPT и другие), а
**«Открыть программу»** выбирает программу прямо из меню «Пуск» на ПК. Такие
кнопки попадают в **панель быстрого запуска**: до семи квадратных кнопок внизу,
когда телефон стоит вертикально, и слева, когда лежит горизонтально. Значок —
символ, логотип, пара букв или иконка самого сайта. Среди логотипов — популярные
сервисы, а также игры и лаунчеры: Steam, Epic Games, Battle.net, EA, Ubisoft,
GOG, Valorant, League of Legends, Counter-Strike, Dota 2, PUBG, Fortnite,
Roblox и другие. Установленная игра получает свой логотип сама.

**Несколько дек.** Если дек больше одной, листай их свайпом вбок на телефоне;
точки под шапкой показывают, где ты.

Когда ПК-сторона отваливается — закрыт агент или сам IT-Deck, — каждая плитка
превращается в кнопку, которая ничего не может, поэтому виджет часов занимает
весь экран в виде семисегментных часов и продолжает показывать время, пока ПК
не вернётся. Дека
восстанавливается сама, переключать ничего не нужно.

Внешний вид — это две настройки: **тема** (Flat, Pastel, Glossy, Liquid Glass)
переключается «таблеткой» в шапке деки, **фон** (авто, светлый, тёмный)
задаётся в Studio. Обе синхронизируются на все подключённые телефоны.

## Studio

`/studio.html` на ПК — кнопка **«Открыть Studio»** в окне. Слева живой
предпросмотр деки, справа редактор: что делает плитка, выбираешь карточками, а
не набором имён команд и JSON; устройства звука подставляются из списка,
который сообщает агент; программы — через **«Выбрать из установленных
программ»** (меню «Пуск» и приложения Microsoft Store); **«Где»** ставит кнопку
в сетку или в панель быстрого запуска; **компактная раскладка** убирает дыры
в сетке. Язык —
английский или русский, по языку браузера. С телефона каталог не меняется.
**«Гайд»** в верхней панели — короткая инструкция с картинками; в первый раз
она открывается сама.

Остальное в верхней панели:
- **«Деки»** — сохранить деку на экране в файл, добавить деку из файла или
  начать с шаблона (**«Стример»**, **«Работа»**). Файл всегда становится
  *новой* декой и не меняет существующую. Открывай только файлы, которым
  доверяешь: плитка может запускать программы на ПК.
- **«Доступ»** — посмотреть и сменить оба токена (см. ниже).
- **«Что нового»** — последние изменения; точка горит, пока не прочитаешь.

### Токены

Оба по умолчанию `admin` — для своего Wi-Fi этого достаточно. Если сеть
общая, смени их:
- **«Новый PIN для телефона»** в окне IT-Deck создаёт случайный 6-значный
  токен телефона. QR-код обновляется, а телефоны со старым токеном один раз
  спросят новый.
- **Studio → «Доступ»** меняет любой из токенов вручную или создаёт случайный
  PIN. Изменение применяется сразу, без перезапуска.

### Настройка плитки VPN

Пока путь не задан, наверху Studio висит карточка **«Укажи путь до
VPN-клиента»**. Вставь полный путь до `.exe` и нажми **«Сохранить»**. Быстрее
всего получить его так: выдели файл в проводнике и нажми **Ctrl+Shift+C** —
кавычки, которые он добавляет, Studio уберёт сама.

После этого плитка подсвечивается всегда, когда программа запущена, — в том
числе до первого нажатия. Нажатие **запускает** VPN; чтобы остановить, зажми
плитку и выбери **Force Stop**.

**Если плитка сообщает, что программа запускается от администратора** — это
самая частая причина «мёртвой» плитки VPN: Windows показывает запрос UAC на
ПК, а у тебя в руках телефон. Либо сними галку **«Запускать эту программу от
имени администратора»** в свойствах exe → «Совместимость», либо запускай сам
`ITDeck.exe` от администратора.

<details>
<summary>Если нужна одна плитка-переключатель</summary>

В редакторе плитки VPN нажми **«Сменить тип»**, выбери **«Программа
вкл/выкл»** и укажи имя процесса (например, `v2RayTun.exe`) и путь до exe.
Тогда одно нажатие запускает, следующее — останавливает. Подтверждения нет,
поэтому по умолчанию не так.

</details>

## Бережно к играм

IT-Deck задуман так, чтобы работать рядом с игрой и не отнимать FPS. Пока ни
один телефон не открыл деку, агент вообще не опрашивает звук и процессы.
Бэкенд и собственная работа агента идут с пониженным приоритетом, так что
занятый процессор в первую очередь достаётся игре; программы, которые
запускает IT-Deck, получают обычный приоритет. Проверка VPN запоминает процесс,
а не перебирает список процессов каждую секунду. Замеренная стоимость, пока
телефон *открыт*: около 0,1 мс работы в секунду (подробности —
[`docs/DEVELOPMENT.md`](docs/DEVELOPMENT.md#13-performance)).

**«Запускать IT-Deck вместе с Windows»** (шаг 3 окна, по умолчанию выключено)
запускает его при входе в систему.

## Обновления

Окно один раз при запуске проверяет страницу релизов и показывает кнопку
**«Скачать»**, если вышла новая версия. Это один запрос, ничего о тебе не
отправляется, любая ошибка молча игнорируется. Выключается через
`UPDATE_CHECK=0` в `config.env`. Устареть может только exe — телефон каждый
раз загружает деку из той версии, которая запущена.

**Настройки переживают обновления.** Плитки, деки, токены и порт хранятся в
`%LOCALAPPDATA%\IT-Deck`, а не в exe: заменил exe — всё на месте. После
обновления окно один раз показывает короткую карточку **«Что нового»**.

## Как удалить

В третьем шаге окна есть неприметная кнопка **«Удалить IT-Deck с этого ПК»**.
Она удаляет сам exe, ярлык с рабочего стола, настройки, токены и раскладку
плиток, правила брандмауэра Windows и временный каталог, в который exe
распаковывается, — это всё, что IT-Deck вообще пишет на диск. Если удалить
только exe, остальные четыре следа останутся.

Сначала он спросит — в окне со списком того, что исчезнет; фокус стоит на
«Отмена», так что Enter и Escape просто закрывают его. Windows один раз
спросит разрешение — оно нужно только для правил брандмауэра, и если
отказаться, всё остальное всё равно удалится. Иконка на домашнем экране
телефона останется — её удали на телефоне.

## Если что-то не работает

| Симптом | Что делать |
| --- | --- |
| QR-код или ссылка не открываются на телефоне | Попробуй другие адреса, которые окно перечисляет под ссылкой — у ПК с VPN, Hyper-V или WSL их несколько |
| Не работает ни один адрес | Брандмауэр Windows: отменённый запрос оставляет правила **«Блокировать»** для `itdeck.exe`. Удали их во входящих правилах или сделай сеть частной |
| Телефон просит токен | Токен сменился (новый PIN или Studio → «Доступ»). Введи новый или отсканируй QR-код заново |
| IT-Deck пишет, что порт уже занят | Уже запущена другая копия. Закрой её кнопкой «Выйти» или заверши `ITDeck.exe` в диспетчере задач |
| Плитка VPN ничего не делает | Скорее всего, у неё ещё нет пути (см. выше). Если путь есть — проверь, не запускается ли программа от администратора |
| Плитка, которой нужен ПК, не срабатывает, или дека превратилась в часы | Агент не запущен. После сбоя он перезапускается сам; после **Close Agent** нажми **«Запустить агент»** в окне. `logs\agent.log` говорит почему, `launcher.log` рядом фиксирует перезапуски |
| Изменение не видно на телефоне | Потяни страницу вниз, чтобы обновить, или закрой и снова открой иконку |
| Вообще ничего не стартует | `%LOCALAPPDATA%\IT-Deck\logs\backend.log` |

## Документация

| Где | Что |
| --- | --- |
| этот файл | Что такое IT-Deck, как поставить и запустить |
| [`CHANGELOG.md`](CHANGELOG.md) | Что изменилось в каждом релизе |
| [`CONTRIBUTING.md`](CONTRIBUTING.md) | **Для разработчиков:** запуск в три команды, проверки перед пушем, правила, которые легко сломать, как делается релиз (на английском) |
| [`docs/DEVELOPMENT.md`](docs/DEVELOPMENT.md) | Как всё устроено и как расширять: схемы, протокол WebSocket, отладка, пошаговые рецепты (на английском) |
| [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) | Почему сделано именно так: архитектурные решения (на английском) |
| [`docs/IT-Deck_Tech_Reference.md`](docs/IT-Deck_Tech_Reference.md) | Полный справочник: архитектура, схема БД, API, протокол, плитки, темы, агент, **standalone (§10)** и техдолг |
| [`docs/legacy-server.md`](docs/legacy-server.md) | Старая схема до v0.3.0: бэкенд в Docker на отдельном сервере и плейбук Ansible |
| [`CLAUDE.md`](CLAUDE.md) | Рабочие заметки для сессий Claude Code: правила и то, что легко сломать |

## Статус

v0.5.7 — личный проект, активная разработка, API может меняться.

## Лицензия

Бесплатно для личного и другого некоммерческого использования по
[PolyForm Noncommercial License 1.0.0](LICENSE): пользуйтесь, читайте код,
меняйте, делитесь. Коммерческое использование — только с разрешения автора.

Что IT-Deck включает в себя или с чем работает (пакеты Python, NirSoft
SoundVolumeView, Open-Meteo, логотипы Simple Icons), перечислено вместе с
условиями в [`THIRD-PARTY-NOTICES.md`](THIRD-PARTY-NOTICES.md). Логотипы —
товарные знаки их владельцев и нужны только чтобы показать, какое приложение
открывает плитка.
